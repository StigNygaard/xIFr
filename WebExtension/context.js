globalThis.context = globalThis.context || (function Context() {
  ...
  function log(...arg) {
    if (LOG && console) console.log(...arg);
  }
  function info(...arg) {
    if (INFO && console) console.info(...arg);
  }
  function debug(...arg) {
    if (DEBUG && console) console.debug(...arg);
  }
  function warn(...arg) {
    if (WARN && console) console.warn(...arg);
  }
  function error(...arg) {
    if (ERROR && console) console.error(...arg);
  }
  ...
  function prefersDark(dispMode) {
    return dispMode === "darkMode" || globalThis?.matchMedia('(prefers-color-scheme: dark)')?.matches;
  }
  function isFirefox() {
...
globalThis.versionnumber = globalThis.versionnumber || (function Versionnumber() {  // major.minor.revision
  ...
  function compare(v1, v2) {
    ...
    for (let i = 0; i < v1parts.length; ++i) {
      if (Number.parseInt(v1parts[i], 10) > Number.parseInt(v2parts[i], 10)) {
        return 1;
      } else if (Number.parseInt(v1parts[i], 10) < Number.parseInt(v2parts[i], 10)) {
        return -1;
      }
    }
    return 0;
  ...
  function parts(v, n) {
    ...
    vparts.map(function (item) {
      return String(Number.parseInt(item.trim(), 10))
    });
