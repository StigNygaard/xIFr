browser.runtime.sendMessage({
  ...
  if (response.data.GPSPureDdLat && response.data.GPSPureDdLon && typeof response.data.GPSPureDdLat.value === 'number' && typeof response.data.GPSPureDdLon.value === 'number') {
    ...
    if (maplinks) {
      ...
      let titleString = encodeURIComponent('Photo location').replace(/_/g, ' '); // Used by Bing. Could potentially be filename or title, but underscores means trouble :-/ ...
      maplinks.insertAdjacentHTML("beforeend", ` <div title="Locate on OpenStreetMap"><a href="https://www.openstreetmap.org/?mlat=${lat}&mlon=${lon}&layers=M">O</a></div>`);
      maplinks.insertAdjacentHTML("beforeend", ` <div title="Locate on Google Maps"><a href="https://www.google.com/maps/search/?api=1&query=${lat},${lon}">G</a></div>`);
      maplinks.insertAdjacentHTML("beforeend", ` <div title="Locate on Bing Maps"><a href="https://www.bing.com/maps/?cp=%lat%~%lon%&lvl=16&sp=point.${lat}_${lon}_${titleString}">B</a></div>`); // var href = 'http://www.bing.com/maps/?cp=%lat%~%lon%&lvl=16&sp=point.%lat%_%lon%_%titleString%_%notesString%_%linkURL%_%photoURL%'; // Can't get it to handle underscores in strings/urls :-/
      maplinks.insertAdjacentHTML("beforeend", ` <div title="Locate on MapQuest"><a href="https://www.mapquest.com/latlng/${lat},${lon}">Q</a></div>`);
      maplinks.insertAdjacentHTML("beforeend", ` <div title="Locate on Here WeGo"><a href="https://share.here.com/l/${lat},${lon}">H</a></div>`);
